...
class Player:
    ...
    def miniMax(self, grid, joueur, adversaire): #score > 0 ==> ia gagne   et vice versa
        global compteur
        if joueur.win():
            return (100,-1)
        elif adversaire.win():
            return (-100,-1)
        elif grid.coupsPossibles() == []:
            return(0,None)
        results = []
        coups = grid.coupsPossibles()
        for coup in coups:
            grid.posePion(coup, joueur)
            score = - adversaire.miniMax(grid,adversaire,joueur)[0]
            results.append((score,coup))
            grid.enlevePion(coup, joueur)
            compteur += 1
            if score == SCOREMAX:
                return (SCOREMAX, coup)
        resultMax = max(results, key = lambda x: x[0])
        return resultMax
...
class Ia(Player):
    def play(self,lagrille,val,game):
        x = self.miniMax(lagrille, game.players[0], game.players[1])[1]
